...
open class LootboxOverwatchAPI {
    ...
    open func getPatchNotes(_ completitionHandler:@escaping (NSArray?, Error?)->()) {
    ...
    open func getProfileStats(_ plataform:String, region:String, tag:String, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
        guard let url = profileStatsURL(plataform, region: region, tag: tag) else { completitionHandler(nil, NSError.noURLFound()); return }
        self.getInfo(url: url, completitionHandler: completitionHandler)
    }
    
    open func getAchievements(_ plataform:String, region:String, tag:String, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
        guard let url = achievementsURL(plataform, region: region, tag: tag) else { completitionHandler(nil, NSError.noURLFound()); return }
        self.getInfo(url: url, completitionHandler: completitionHandler)
    }
    
    open func getHeroesStats(_ plataform:String, region:String, tag:String, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
        guard let url = heroesStatsURL(plataform, region: region, tag: tag) else { completitionHandler(nil, NSError.noURLFound()); return }
        self.getInfo(url: url, completitionHandler: completitionHandler)
    }
    
    open func getHeroStats(_ plataform:String, region:String, tag:String, hero:String, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
    ...
    open func getOverallHeroesStats(_ plataform:String, region:String, tag:String, completitionHandler:@escaping (NSArray?, Error?)->()) {
    ...
    open func getPlataforms(_ plataform:String, region:String, tag:String, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
        guard let url = plataformsURL(plataform, region: region, tag: tag) else { completitionHandler(nil, NSError.noURLFound()); return }
        self.getInfo(url: url, completitionHandler: completitionHandler)
    }
    
    // MARK: - Private Methods
    fileprivate func getInfo(url:URL, completitionHandler:@escaping (NSDictionary?, Error?)->()) {
		URLSession.dataTask(with: url) { (data, response, error) in
            URLSession.validateURLSessionDataTask(data, response: response, error: error as NSError?, completitionHandler: { (data, error) in
                if (error == nil) {
                    completitionHandler(JSONSerialization.jsonObject(with: data), nil)
                } else {
    ...
    fileprivate func getInfo(url:URL, completitionHandler:@escaping (NSArray?, Error?)->()) {
        URLSession.dataTask(with: url) { (data, response, error) in
            URLSession.validateURLSessionDataTask(data, response: response, error: error as NSError?, completitionHandler: { (data, error) in
                if (error == nil) {
                    completitionHandler(JSONSerialization.jsonObject(with: data), nil)
